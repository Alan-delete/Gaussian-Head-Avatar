...
#     load MODNET model for silhouette masks
    ...
    for i in tqdm(range(len(tens_list))):
        ...
        cv2.imwrite(images[i].replace('images', 'masks/body'), postprocess_mask(silh_mask)[0].astype(np.uint8))
...
#     load CDGNet for hair masks
    ...
    for image in images:
        ...
        mask = np.asarray(Image.open(image.replace('images', 'masks/body')))
        images.append(img)
        masks.append(mask)
    ...
    for i in range(len(images)):
        ...
        Image.fromarray(hair_mask).save(images[i].replace('images', 'masks/hair'))
   
    print('Results saved in folder: ', os.path.join(data_dir, 'masks'))

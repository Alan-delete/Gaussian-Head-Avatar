...
def preprocess_renderme(args, data_folder):
    device = torch.device(args.device)
    # Load model
    if args.model_type == 'mattingbase':
        model = MattingBase(args.model_backbone)
    ...
    fids = sorted(os.listdir(os.path.join(data_folder, 'images')))
    image_paths = sorted(glob.glob(os.path.join(data_folder, 'images', '*', '*.jpg')))
    for image_path in image_paths:
        for fid in tqdm(fids):
            # image_path = os.path.join(data_folder, 'images', fid, 'image_%s.jpg' % camera_ids[v])
            background_path = image_path.replace("images", "backgrounds")
            if not os.path.exists(image_path):
                continue
            image = imageio.imread(image_path)
            src = (torch.from_numpy(image).float() / 255).permute(2,0,1)[None].to(device, non_blocking=True)
            ...
            imageio.imsave(mask_path, mask)
            # mask_lowres_path = os.path.join(data_folder, 'images', fid, 'mask_lowres_%s.jpg' % camera_ids[v])
            mask_lowres_path = image_path.replace("image_", "mask_lowres_")
            imageio.imsave(mask_lowres_path, mask_lowres)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='../datasets/NeRSemble')
    parser.add_argument('--dataset', type=str, default='nersemble', choices=['nersemble', 'renderme'])
    ...
    # ids = sorted(os.listdir(args.data_path))
    # for id in ids:
    #     data_folder = os.path.join(args.data_path, id)

    data_folder = args.data_path
    if args.dataset == 'nersemble':
        preprocess_nersemble(args, data_folder, CAMERA_IDS)
    else:
        preprocess_renderme(args, data_folder)

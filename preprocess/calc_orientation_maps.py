...
def main(args):
    ...
    for frame in frames_names:
        os.makedirs(os.path.join(args.orient_dir, frame), exist_ok=True)
        os.makedirs(os.path.join(args.conf_dir, frame), exist_ok=True)
    ...
    for file_path, mask_path in tqdm.tqdm(zip(filepaths, mask_paths), total=len(filepaths)):
        img_name = file_path.split('/')[-1]
        flame = file_path.split('/')[-2]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mask_dir', default='../datasets/mini_demo_dataset/031/masks/hair', type=str)
    parser.add_argument('--orient_dir', default='../datasets/mini_demo_dataset/031/orientation_maps', type=str)
    parser.add_argument('--conf_dir', default='../datasets/mini_demo_dataset/031/orientation_confidence_maps', type=str)
    parser.add_argument('--filtered_img_dir', default='../datasets/mini_demo_dataset/031/orientation_filtered_imgs', type=str)
    parser.add_argument('--vis_img_dir', default='../datasets/mini_demo_dataset/031/orientation_vis_imgs', type=str)
